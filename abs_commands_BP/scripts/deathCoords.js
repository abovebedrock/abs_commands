...
import { dimensionLocalezhCN, getCoordinate } from "./common/common";
...
registerCommand({
    ...
    callback: (_name, player)=>{
        if(player.getDynamicProperty("eligibleForDeathCoord") === true) player.sendMessage("§c你已经开启了死亡坐标提示功能，无需再次开启！");
            ...
            const hand = player.getComponent(EntityComponentTypes.Equippable).getEquipmentSlot(EquipmentSlot.Mainhand), item = hand.getItem();
            ...
            else player.sendMessage("§c错误：你没有手持追溯指针！");
        }
        return true;
    }
});

...
import { fireCommand, prefixs, registerCommand } from "./common/commandBase";
...
const chainedCommandsQuota = new Map(getABSPlayers().map(value=>[value, 10]));
        ...
        if(index > maxFsts) player.sendMessage(`最多只有${maxFsts}条常用语！`);
            ...
            if(fst !== undefined && fst !== ""){
                ...
                if(fst[0] === "/" && availableCommands.includes(fst.substring(1, fst.indexOf(" ") === -1 ? fst.length : fst.indexOf(" ")))){
                    ...
                    const quota = /**@type {number}*/ (chainedCommandsQuota.get(player.name));
                    if(quota > 0) system.run(()=>{
                        fireCommand(fst, player);
                        chainedCommandsQuota.set(player.name, quota - 1);
                    });
                    else{
                        player.sendMessage("§c错误：你通过常用语链式执行的命令过多！");
                        system.runTimeout(()=>chainedCommandsQuota.set(player.name, 10), 100);
                    }
                }
                else{

...
import { dimensionLocalezhCN, getBiome, getCoordinate, getTimeString, separator, updateSeparator } from "./common/common";
...
registerCommand({
    ...
    document: "放心，会完全自动更新所有东西的分隔符的！但是做新的东西的时候要小心",
    tagsRequired: ["dev"],
    args: [{
        name: "setValue",
        optional: true,
    ...
    callback: (_name, player, args)=>{
        if(args.setValue){
            ...
            if(data.length > 1){
                let str = "", succeed = true;
                for(let i = 0; i < data.length; i++){
                    const int = parseInt(data[i]);
                ...
                if(succeed){
                    player.sendMessage(`新分隔符： ${str} （${data.join(",")}）`);
                    updateSeparator(str);
                    return true;
                }
            }
            player.sendMessage("§c搞的啥子哦！至少要有两个！而且是正常的数字！");
        }
        else{
            const result = [];
...
function showMain(player){
    ...
    mainForm.show(player).then(response=>{
        if(response.cancelationReason === FormCancelationReason.UserBusy) system.run(()=>showMain(player));
            ...
            else player.sendMessage("§e目前无未读留言。");
        }
...
function showSendSelect(player){
    const
        filteredPlayers = ["§7", ...getABSPlayers().filter(value=>!world.getPlayers({name: value}).length)],

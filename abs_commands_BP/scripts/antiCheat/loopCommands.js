...
import { ban } from "./ban";
import { retrieveDimension } from "../common/common";
...
world.afterEvents.playerSpawn.subscribe(data=>{
    if(data.initialSpawn){
        data.player.addTag("skiploop");
        system.runTimeout(()=>{
            if(world.getAllPlayers().includes(data.player)) data.player.removeTag("skiploop");
        }, 40);
    }
});
...
function removeSurvival(ids, banPlayer){
    const players = world.getPlayers({excludeGameModes: [GameMode.creative, GameMode.spectator], excludeTags: ["skiploop"]});

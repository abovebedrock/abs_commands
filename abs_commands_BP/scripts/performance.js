...
export async function getPerformanceString(textColor){
    ...
    return `§${textColor ?? "f"}TPS： §e${tps > 20 ? "20.0" : tps.toFixed(1)}§${textColor ?? "f"}， MSPT： §e${mspt < 50 ? "(<50)" : mspt.toFixed(2)}§${textColor ?? "f"}， ${ping !== undefined ? `Ping： §e${ping.toFixed(0)}ms§${textColor ?? "f"}` : "Ping暂时无法测量"}。§r`;
}

...
world.afterEvents.playerSpawn.subscribe(data=>{
    ...
    if(data.initialSpawn) system.runTimeout(async ()=>{
        ...
        for(let i = 0; i < players.length; i++) if(players[i] && players[i].name === name && players[i].getDynamicProperty("disableWelcome") !== true) players[i].sendMessage(`§6§l基岩之上服务器第${meta.season}季\n§r${performanceStr}\n§b欢迎玩家${name}，输入${prefixs[0]}h查看命令帮助，输入${prefixs[0]}j false关闭本进服提示。`);
    }, 60);
});
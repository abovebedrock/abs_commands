...
/**
 * @typedef {Object} Command 命令存储数组
 * @property {string[]} names
 * @property {argumentDef[]} args
 * @property {string} description
 * @property {(usedName :string, player :Player, args :Record<string, (string | number | boolean)>)=>boolean | Promise<boolean>} callback
 * @property {string[]} [tagsRequired]
 * @property {boolean} [hidden] **不要用这个属性！**
...
registerCommand({
    ...
    callback: (_name, player, args)=>{
        ...
        if(message.length === 3){
            ...
            player.sendMessage(`成功获取留言。已经自动将分隔符转换为#。`);
        }
        else{
